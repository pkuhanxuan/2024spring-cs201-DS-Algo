...
def build_graph(filename):
    ...
    for line in all_words:
        word = line.strip()
        for i, _ in enumerate(word):  #0,b 1,a 2,n 3,e 遍历字符串word的每个字符，i是字符的索引，从0开始
            bucket = f"{word[:i]}_{word[i + 1:]}"  # 创建一个新的字符串，将word的第i个字符替换为下划线'_'
            buckets.setdefault(bucket, set()).add(word)  # 在字典buckets中，找到键为bucket的项，如果找不到，就创建一个新的项，键为bucket，值为一个空的集合。然后，将word添加到这个集合中。
...
# 并为每个顶点着色、赋距离和前驱。
bfs(g.get_vertex("FOOL")) #get_vertex是Graph类的方法，返回字典vertices中键为"FOOL"的值，即Vertex类的对象
...
```python
from collections import defaultdict
# defaultdict是Python的一个内置字典类，它继承自内置的dict类。defaultdict的主要特性是它会自动为字典中不存在的键提供一个默认值。

# 在这段代码中，defaultdict被用来创建一个字典，该字典的键是图中的顶点，值是一个列表，表示与该顶点相连的其他顶点。当你试图访问一个不存在的键时，defaultdict会自动为这个键创建一个空列表作为默认值，然后返回这个空列表。这样，你就可以直接向这个列表中添加元素，而不需要先检查这个键是否存在。

# 例如，self.graph[u].append(v)这行代码就是向顶点u的邻接列表中添加顶点v。如果顶点u还没有在字典中，defaultdict会自动为它创建一个空列表，然后将顶点v添加到这个列表中。

# 这是defaultdict【默认字典】的一个常见用法，它可以简化代码，避免需要手动检查键是否存在的情况。
...
>                 print("FOUND:", neigh)
>                 print(path)
>                 sys.exit(0) #在Python中，sys.exit(0)是用来退出程序的。sys.exit函数接受一个可选的参数，这个参数通常是一个整数，用来表示程序的退出状态。如果程序正常退出，通常使用0作为退出状态。如果程序因为某种错误而退出，通常使用非零的退出状态。
> print("Not found")
> print(path)
> """
> FOUND: J
...
def knight_tour(n, path, u, limit):
    ...
    if n < limit:
        ...
        for nbr in neighbors:
            if nbr.color == "white" and knight_tour(n + 1, path, nbr, limit):   #选择“白色”未经深入的点，层次加一，递归深入
                return True
        else:                       #所有的“下一步”都试了走不通
...
def dijkstra(graph,start):
    ...
    # 将起始顶点的距离设置为0
    start.setDistance(0)
    # 将图中的每个顶点及其距离作为一个元组添加到优先队列中，并构建一个堆
    pq.buildHeap([(v.getDistance(),v) for v in graph])
    # 只要优先队列不为空，就会继续循环
    while pq:
        # 从优先队列中删除并返回具有最小距离的顶点
        ...
        for next_v in current_v.getneighbors():
            ...
            if new_distance < next_v.distance:
                # 如果新距离更小，将邻居顶点的距离更新为新距离
                next_v.distance = new_distance
                # 将邻居顶点的前一个顶点设置为当前顶点
                next_v.previous = current_v
                # 在优先队列中改变邻居顶点的优先级，新的优先级是新距离
...
class DisjointSet:
    ...
    def find(self, x):
        # 如果x的父节点不是自己，那么就递归地找x的父节点的父节点，直到找到一个节点是自己的父节点
        # 这个过程中，将x的父节点设置为其父节点的父节点，实现路径压缩
        if self.parent[x] != x:
    ...
    def union(self, x, y):
        ...
        # 如果x和y的根节点不同，那么就需要将它们合并
        if root_x != root_y:
            # 如果x的秩小于y的秩，那么就将x的根节点的父节点设置为y的根节点
            if self.rank[root_x] < self.rank[root_y]:
                self.parent[root_x] = root_y
            # 如果x的秩大于y的秩，那么就将y的根节点的父节点设置为x的根节点
            elif self.rank[root_x] > self.rank[root_y]:
                self.parent[root_y] = root_x
            # 如果x和y的秩相同，那么就随便选择一个作为父节点，并将其秩增加1
            else:
                self.parent[root_x] = root_y
                self.rank[root_y] += 1
...
def kruskal(graph):
    ...
    # 构建边集
    # 遍历图的每个顶点，将每个顶点与其后的所有顶点之间的边添加到边集中
    for i in range(num_vertices):
        for j in range(i + 1, num_vertices):
            if graph[i][j] != 0:
                edges.append((i, j, graph[i][j]))
    ...
    # 按照权重排序边集
    # 使用lambda函数将边的权重作为排序的关键字
    edges.sort(key=lambda x: x[2])

    # 初始化并查集
    # 并查集中的每个元素都是一个集合，初始时每个集合只包含一个元素
    disjoint_set = DisjointSet(num_vertices)

    # 构建最小生成树的边集
    # 遍历排序后的边集，如果一条边的两个顶点不在同一个集合中，就将这条边添加到最小生成树的边集中，并将这两个顶点所在的集合合并
    minimum_spanning_tree = []
    for edge in edges:
        u, v, weight = edge
        if disjoint_set.find(u) != disjoint_set.find(v):
